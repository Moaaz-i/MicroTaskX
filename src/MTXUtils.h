...
public:
  ...
  template <uint8_t PIN> static inline void toggleFast()
  {
#if defined(__AVR__)
  ...
  static bool isButtonLongPressed(uint8_t buttonPin, MTXButton& btnState,
  ...
  static uint8_t calculateBatteryPercentage(uint8_t analogPin, float r1, float r2, float minVolts,
                                            float maxVolts, float vRef = 5.0,
  ...
  static void updateScreenSmart(T& lcd, const char newBuf[ROWS][COLS + 1],
                                char oldBuf[ROWS][COLS + 1])
  {
...
template <typename T, uint16_t SIZE> class MTXCircularQueue
{
private:
  T _buffer[SIZE];
  uint16_t _head = 0;
